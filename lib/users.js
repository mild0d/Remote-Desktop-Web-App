...
const VALID_THEMES = ['dark', 'light', 'win11', 'win10', 'winxp'];

// Defaults to 'dark' - the app's original, only appearance before this
// feature existed - so accounts that predate it see no visual change at
// all until someone actively picks something else.
function getUserTheme(userId) {
  const user = findById(userId);
  if (user && VALID_THEMES.includes(user.theme)) return user.theme;
  return 'dark';
}
...
function setUserTheme(userId, theme) {
  ...
  const idx = list.findIndex((u) => u.id === userId);
  if (idx === -1) throw new Error('User not found');
  list[idx].theme = theme;
  saveAll(list);
}

const MIN_IDLE_TIMEOUT_MINUTES = 5;
const MAX_IDLE_TIMEOUT_MINUTES = 480; // 8 hours
const DEFAULT_IDLE_TIMEOUT_MINUTES = 60; // 1 hour

// Entirely user-controlled, no admin involvement - defaults to 1 hour for
// both new and existing accounts, since idle-timeout didn't exist at all
// before this feature.
function getIdleTimeoutMinutes(userId) {
  const user = findById(userId);
  if (user && Number.isFinite(user.idle_timeout_minutes)) return user.idle_timeout_minutes;
  return DEFAULT_IDLE_TIMEOUT_MINUTES;
}

function setIdleTimeoutMinutes(userId, minutes) {
  const parsed = Number(minutes);
  if (!Number.isFinite(parsed) || parsed < MIN_IDLE_TIMEOUT_MINUTES || parsed > MAX_IDLE_TIMEOUT_MINUTES) {
    const err = new Error(`Idle timeout must be between ${MIN_IDLE_TIMEOUT_MINUTES} and ${MAX_IDLE_TIMEOUT_MINUTES} minutes`);
    err.code = 'INVALID_IDLE_TIMEOUT';
    throw err;
  }
  const list = loadAll();
  const idx = list.findIndex((u) => u.id === userId);
  if (idx === -1) throw new Error('User not found');
  list[idx].idle_timeout_minutes = Math.round(parsed);
  saveAll(list);
}
...
function listAllUsers() {
  return loadAll().map((u) => {
    ...
    return {
      ...
      is_sso: Boolean(u.sso_subject),
      sso_2fa_required: Boolean(u.sso_2fa_required),
...
// Per-account, admin-controlled - separate from whether 2FA is actually
// set up (totp_enabled). Turning this off only stops requiring the code
// at login; it deliberately never touches the person's actual 2FA setup,
// so re-enabling it later works immediately without them redoing
// anything. Meaningless for local accounts, which already require 2FA
// unconditionally regardless of this flag.
function getSSOTwoFactorRequired(userId) {
  const user = findById(userId);
  return Boolean(user && user.sso_2fa_required);
}

function setSSOTwoFactorRequired(userId, required) {
  const list = loadAll();
  const idx = list.findIndex((u) => u.id === userId);
  if (idx === -1) throw new Error('User not found');
  if (!list[idx].sso_subject) {
    const err = new Error('This setting only applies to SSO accounts');
    err.code = 'NOT_SSO_ACCOUNT';
    throw err;
  }
  list[idx].sso_2fa_required = Boolean(required);
  saveAll(list);
}
