...
function getConfig() {
  ...
  return {
    ...
    baseDN: raw.baseDN,
    // CA certificates are public by nature (that's the whole point of a
    // CA) - no encryption needed, unlike the bind password.
    caCert: raw.caCert || '',
    skipCertValidation: Boolean(raw.skipCertValidation),
  };
}
...
function setConfig({ url, bindDN, bindPassword, baseDN, caCert, skipCertValidation }) {
  ...
  const updated = {
    ...
    caCert: caCert || '',
    skipCertValidation: Boolean(skipCertValidation),
  };
  saveRaw(updated);
}
